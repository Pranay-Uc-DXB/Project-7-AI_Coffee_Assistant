...
def main():
    ...
    #This is the main loop that continuously listens for audio input.
    try:
        while True:
            ...
            # Record audio chunk
            print("_")
            #If the recorded chunk is not silence, it transcribes the audio and prints the customer's speech.
            if not record_audio_chunk(audio, stream):
                ...
                # Process customer input and get response from AI assistant (Coverting it into speech)
                output = ai_assistant.interact_with_llm(transcription)
                if output:                    
                    output = output.lstrip()
                    vs.multilingual_text_to_speech(output, 'hindi')
